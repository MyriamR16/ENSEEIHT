...
for k=1:length(EbN0db)
    rapp = EbN0(k);
    sigma = mean(abs(I_t).^2)*Ns/(4*log2(4)*rapp);
    bruit = sqrt(sigma)*randn(1,length(I_t));
    I_t_bruit = I_t + bruit;
    Q_t_bruit = Q_t + bruit;
    ...
    signal_porteuse_bruit = signal_phase_porteuse - signal_quadrature_porteuse;
