...
for i = 1:k
  ...
  % Perte d'orthogonalite avec algorithme classique bis
  Qcgscgs = cgs(Qcgs)
  po(3,i) = norm(eye(n)-Qcgscgs'*Qcgscgs);
  
  % Perte d'orthogonalite avec algorithme modifie bis
  Qmgsmgs = mgs(Qmgs)
...
figure('Position',[0.1*L,0.1*H,0.8*L,0.75*H])
    ...
    leg = legend('Gram-Schmidt classique',...
                 ...
                 'Gram-Schmidt classique classique',...
                 'Gram-Schmidt modifie modifie',...
                 'Location','NorthWest');
    set(leg,'FontSize',14);
    xlim([x(1) x(end)])
