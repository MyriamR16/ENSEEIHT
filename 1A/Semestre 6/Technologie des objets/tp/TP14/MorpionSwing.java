...
// ---------------
	...
	public MorpionSwing(ModeleMorpion modele) {
		...
		this.modele = modele;
		Container grilleJeu = fenetre.getContentPane(); 
		grilleJeu.setLayout(new GridLayout(3, 3));
		// Créer les cases du Morpion
		for (int i = 0; i < this.cases.length; i++) {
			for (int j = 0; j < this.cases[i].length; j++) {
				this.cases[i][j] = new JLabel();
				grilleJeu.add(cases[i][j]);
			}
		}
		...
		JMenuItem menu1Item1 = new JMenuItem("Nouvelle Partie");
		JMenuItem menu1Item2 = new JMenuItem("Quitter");
		menu1.add(menu1Item1);
		menu1.add(menu1Item2);
		JMenuBar barMenue = new JMenuBar(); 
		barMenue.add(menu1);

		//Rajout du menu dans la fenetre
		this.fenetre.setJMenuBar(barMenue);

		JButton quitter = new JButton("Q");
		JButton nouvellePartie = new JButton("N");

		this.fenetre.add(quitter,BorderLayout.EAST);
		this.fenetre.add(nouvellePartie, BorderLayout.WEST);
