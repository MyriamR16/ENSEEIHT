...
procedure Evaluer_Alea_TH is
    ...
    is
        ...
        package TH_String_Integer is
                new TH (T_Cle => Integer, T_Valeur => Integer, Capacite=>Borne, fonction_de_hachage=>fonction_de_hachage);
    ...
    begin
        ...
        for I in 1..Borne loop
            Enregistrer (Tableau,I, 0);
        end loop;

        for J in 1..Taille loop
            Get_Random_Number (Nombre_alea);
            E := La_Valeur(Tableau, Nombre_alea);
            E:= E+1;
            Enregistrer(Tableau,Nombre_alea, E);
      end loop ;
      Max:=0;
      Min:=Taille;
      for J in 1..Borne loop
         E := La_Valeur (Tableau, J);
         if E<Min then
             Min:=E;
         elsif E>Max then
             Max:=E;
