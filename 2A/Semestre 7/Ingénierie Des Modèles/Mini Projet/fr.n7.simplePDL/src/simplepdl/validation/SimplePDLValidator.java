...
public class SimplePDLValidator extends SimplepdlSwitch<Boolean> {
	...
	public Boolean caseProcess(simplepdl.Process object) {
		...
		for (ProcessElement pe : object.getProcessElements()) {
			...
			if (pe instanceof WorkDefinition) {
				...
				for (Occurrence occ : wd.getOccurrence()) {
					this.doSwitch(occ);
				}
			}
		}
	...
	public Boolean caseWorkSequence(WorkSequence object) {
		...
		this.result.recordIfFailed(
				object.getPredecessor().getLinksToSuccessors().stream()
					.filter(p -> p.getLinkType() == object.getLinkType())
					.filter(p -> p.getSuccessor()== object.getSuccessor())
					.count() == 1,
				object, 
				"Le lien :" + object.getPredecessor().getName() + " " + object.getLinkType().getName() + " " +object.getSuccessor().getName() + " existe deja !");
	...
	public Boolean caseRessource(Ressource object) {
		...
		this.result.recordIfFailed(
				...
				object,
				"Attention, la quantite de ressource ne peut pas etre negative !");
		return null;
	}
	
	@Override
	public Boolean caseOccurrence(Occurrence object) {
	    this.result.recordIfFailed(
	            object.getOccurrence() >= 0,
	            object,
	            "Attention, la quantité d'occurrences doit être positive !");
	    
	    this.result.recordIfFailed(
	            object.getOccurrence() <= object.getRessource().getQuantite(),
	            object,
	            "Attention, la quantité d'occurrences ne peut pas dépasser la quantité de ressources disponibles !");
