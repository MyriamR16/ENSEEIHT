...
function regions_de_confiance(f::Function, gradf::Function, hessf::Function, x0::Vector{<:Real};
    ...
    while NonArret 
        g = gradf(x_sol)
        if algo_pas == "cauchy"
            sk = cauchy(g, hessf(x_sol), Δk)
        elseif algo_pas == "gct"
            sk = gct(g, hessf(x_sol) , Δk;  max_iter = max_iter_gct)
        end

        msk = f(x_sol) + transpose(g)*sk + (1/2)*transpose(sk)*hessf(x_sol)*sk
        m0 = f(x_sol)
        rk = (f(x_sol) - f(x_sol + sk))/(m0 - msk)
        xk = x_sol
        ...
        if rk >= η1 
            ...
            elseif (norm(f(x_sol) - f(xk)) <= epsilon * max(tol_rel * norm(f(xk)) , tol_abs))
                flag = 2
                NonArret = false
            end
        end
        ...
        if (nb_iters == max_iter) 
            flag = 3
            NonArret = false
        elseif (norm(gradf(x_sol))) <= max(tol_rel * norm(gradf(x0)) , tol_abs)
            flag = 0 
            NonArret = false
        end
    end 
