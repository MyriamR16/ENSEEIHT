...
function newton(f::Function, gradf::Function, hessf::Function, x0::Union{Real,Vector{<:Real}}; 
    ...
    while (NonArret)
          ...
          if (norm(gradf(x_kp1))) <= max(tol_rel * norm(gradf(x0)) , tol_abs)
            flag = 0 
            NonArret = false

          elseif (norm(x_kp1 - x_sol) <= epsilon * max(tol_rel * norm(x_sol) , tol_abs))
            flag = 1
            NonArret = false

          elseif (norm(f(x_kp1) - f(x_sol)) <= epsilon * max(tol_rel * norm(f(x_sol)) , tol_abs))
            flag = 2
            NonArret = false
