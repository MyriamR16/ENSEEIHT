...
(******************************************************************************)
let rec appartient mot (Trie(Noeud(b,lb), fd, fr)) = 
    let listeMot = fd mot in 
    (*appartient_arbre listeMot a*)
    match listeMot, lb with 
    | [],_ -> b 
    | _, [] -> false 
    | tc::qc, (tb,Noeud(b1,lb1))::qb -> if tc=tb then (appartient (fr qc) (Trie(Noeud(b1,lb1),fd,fr)))
    else if tc<tb then false 
    else appartient (fr listeMot) (Trie(Noeud(b,qb),fd,fr))

let%test _ = appartient "bas" (Trie(arbre_sujet2, decompose_chaine, recompose_chaine))
let%test _ = not(appartient "patatefrite" (Trie(arbre_sujet2, decompose_chaine, recompose_chaine)))
...
let%test_unit _ = affiche affiche_chaine (Trie(arbre_sujet,decompose_chaine,recompose_chaine))