...
let rec retrait_arbre lc (Noeud (b,lb)) =
   ...
   | c::qlc ->
      let arbre_c =
        let l = recherche c lb in
        match l with
        | None   -> failwith "Le mot que vous voulez enlever n'existe pas"
        | Some a -> a
...
let parcours arbre = 
   let rec parcours_arbre(Noeud(b,lb)) = 
      ...
      | _ -> 
         if b then
            []::List.flatten (List.map parcours_branche lb)
         else
            List.flatten (List.map parcours_branche lb)
   and parcours_branche (c,arbre) = 
      List.map (fun l -> c::l) (parcours_arbre arbre)
   in parcours_arbre arbre

let normalisation (Noeud(b,lb)) =
   let rec listesAFournir lb = 
      match lb with 
      | [] -> []
      | t::q -> (parcours_branche(t)) @ (listesAFournir(q))
   and parcours_branche (c,Noeud(b1,lb1)) =
      let arbre = Noeud(b1, (listesAFournir lb1)) in
      match arbre with 
      | Noeud(false,[]) -> []
      | _ -> [(c, arbre)]
   in Noeud(b, (listesAFournir lb))
