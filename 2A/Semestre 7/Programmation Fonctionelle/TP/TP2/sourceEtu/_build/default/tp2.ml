...
(* let tri_insertion ordre l = List.fold_right (fun t q -> insert ordre t q) l [] *)
let tri_insertion ordre l = List.fold_left (fun acc t -> insert ordre t acc ) [] l
...
let scinde2 l =
  ...
  let rec aux indice l1 l2 =
    if indice >= taille then l1,l2
    else 
      match l1 with 
      | [] -> l1,l2
      | t::q -> aux (indice+1) q (t::l2)
    in aux 0 l []
...
let fusionne2 ordre l1 l2 = 
  let rec aux l1 l2 l=
    ...
    | [] , t::q -> aux l1 q (t::l)
    | t::q , [] -> aux q l2 (t::l)
    | t1::q1 , t2::q2 -> if (ordre t1 t2) then (aux q1 l2 (t1::l))
    else (aux l1 q2 (t2::l))
  in aux l1 l2 []
...
let tri_listeStat listeStat = tri_fusion (fun (_,_,_,x) (_,_,_,y) -> x > y ) listeStat
let tri_listeStatSort listeStat = List.sort (fun (_,_,_,x) (_,_,_,y) -> compare y x) listeStat
*)