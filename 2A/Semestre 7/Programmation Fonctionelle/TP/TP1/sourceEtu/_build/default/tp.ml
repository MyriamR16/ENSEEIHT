...
let padovan2 n = 
   let rec aux u0 u1 u2 nn =
      match nn with 
      | 0 -> u0
      | 1 -> u1
      | 2 -> u2
      | _ -> aux u1 u2 (u0+u1) (nn-1)
   in aux 0 0 1 n
...
let estPremier n = 
   ...
   | _ -> 
   let rec aux indice = 
      if indice >= n then true 
      else if (n mod indice) = 0 then false 
      else aux (indice+1)
   in aux 2
...
let rec dragon (xa,ya) (xb,yb) n = 
   if n < 0 then failwith "n doit etre positif attention !"
   else if n=0 then dessine_segment (xa,ya) (xb,yb)
   else let (xc, yc) = (((xa+xb)/2) + ((yb-ya)/2) , ((ya+yb)/2) + ((xa-xb)/2)) in 
